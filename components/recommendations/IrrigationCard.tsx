...
const IrrigationCard: React.FC<IrrigationCardProps> = ({
  ...
  const getPersonalizedMessage = () => {
    ...
    if (weatherData.isRaining || (weatherData.humidity && weatherData.humidity > 95)) {
      return `${greeting}${name}${emoji} Pas d'irrigation recommandée aujourd'hui. La pluie suffit.`;
    }
    
    if (weatherData.humidity && weatherData.humidity > 80) {
      // Humidité élevée - volume divisé par 2
      return `${greeting}${name}${emoji} Apportez ${(volume/2).toFixed(1)} L/m² à vos ${culture} (sol ${sol}) — soit ${Math.round(totalVolume/2)} L au total.`;
    }

    if (weatherData.temperature && weatherData.temperature > 38) {
      return `${greeting}${name}${emoji} Apportez ${volume.toFixed(1)} L/m² à vos ${culture} (sol ${sol}) — soit ${Math.round(totalVolume)} L au total.`;
    }
    
    if (weatherData.rainForecast && isMorning()) {
      return `${greeting}${name}${emoji} Apportez ${volume.toFixed(1)} L/m² à vos ${culture} (sol ${sol}) — soit ${Math.round(totalVolume)} L au total. Arrosez ce matin avant la pluie.`;
    }
  ...
  const getFrequencyMessage = () => {
    if (frequency.toLowerCase().includes('24h') || frequency.toLowerCase().includes('24 h') || frequency.toLowerCase().includes('jour')) {
      return "CAS NORMAL – arroser 1 fois par jour (toutes les 24h)";
    }
    return frequency;
  };
  ...
  const getConstraints = () => {
    ...
    if (weatherData.isRaining) {
      constraints.push({
        icon: 'rainy', 
        message: "Il pleut actuellement. Pas d'irrigation nécessaire.",
        type: 'danger'
      });
    }
    ...
    if (weatherData.humidity && weatherData.humidity > 80) {
      constraints.push({
        icon: 'water-outline',
        message: "Taux d'humidité > 80% ! La quantité d'eau sera la ½ de la normale",
        type: 'warning'
      });
    }
    ...
    if (weatherData.temperature && weatherData.temperature > 38) {
      constraints.push({
        icon: 'thermometer',
        message: "Température trop élevée ! Préférez un arrosage très tôt le matin ou tard le soir.",
        type: 'danger'
      });
    }
    ...
    if (weatherData.rainForecast) {
      if (isMorning()) {
        constraints.push({
          icon: 'cloudy',
          message: "⚠️ Pluie prévue ce soir. Arrosez ce matin avant la pluie.",
          type: 'rain-alert'  // Type spécial pour la pluie prévue
        });
      } else {
        constraints.push({
          icon: 'cloudy',
          message: "⚠️ Pluie prévue pour demain. Aujourd'hui, arrosez normalement.",
          type: 'rain-alert'  // Type spécial pour la pluie prévue
        });
      }
    }
    
    // Ajouter la contrainte passée en prop si elle n'est pas déjà gérée
